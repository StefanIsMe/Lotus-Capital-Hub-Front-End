...
const ContributeModal: React.FC<Props> = ({
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      ...
      onConfirm: () => {
        console.log(valueWithTokenDecimals.toString(),poolId);
        return contract.depositPool(valueWithTokenDecimals.toFixed(0), poolId === PoolIds.poolBasic ? 0 : 1, {
          gasPrice,
        })
      },
      onSuccess: async () => {
      ...
      <ModalBody maxWidth="320px">
        ...
        <Flex justifyContent="space-between" mb="8px">
          ...
          <Flex flexGrow={1} justifyContent="flex-end">
            <Image
              src={`/images/farms/${currency.address[56]}.svg`}
              width={24}
              height={24}
            />
          ...
          {multiplierValues.map((multiplierValue, index) => (
            <Button
              ...
              onClick={() => {
                ...
                setValue(getBalanceAmount(maximumLpCommitable.times(multiplierValue)).toString())
              }
              }
